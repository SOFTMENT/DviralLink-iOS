...
class AddStudioController : UIViewController {
    ...
    var studioModel : StudioModel = StudioModel()
    ...
    override func viewDidLoad() {
        ...
        id = Firestore.firestore().collection("Studios").document().documentID
        studioModel.id = id
    ...
    @IBAction func addBtnClicked(_ sender: Any) {
        ...
        if sPrice == "" {
            ...
            self.studioModel.image = downloadURL
            self.studioModel.name = sName
            self.studioModel.address = sLocation
            self.studioModel.price = Int(sPrice ?? "1")!
            try? Firestore.firestore().collection("Studios").document(self.id).setData(from: self.studioModel,completion: { error in
                self.progressHUDHide()
                if error == nil {
                    self.showToast(message: "Studio Added")
...
extension AddStudioController : UIImagePickerControllerDelegate, UINavigationControllerDelegate,CropViewControllerDelegate {
    ...
    func cropViewController(_ cropViewController: CropViewController, didCropToImage image: UIImage, withRect cropRect: CGRect, angle: Int) {
        ...
        uploadImageOnFirebase(storeId: id){ downloadURL in
